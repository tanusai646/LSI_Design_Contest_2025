...
*/
/*20250130-001の変更内容
ハードウェアを最低限利用できるようになったので、プログラムを多少修正
512×512の画像を実際に利用する
*/
...
void block_cal(
	double w2_mean[][NUM_X], double b2_mean[NUM_A3],
	double w2_var[][NUM_X], double b2_var[NUM_A3],
	double k[][NUM_A2],
	double w3[][NUM_A3], double b3[NUM_IN], double eps[NUM_IN],
	double psnr[1])
{
	double z2_mean_tmp[NUM_A3], a2_mean_tmp[NUM_A3];
	double z2_var_tmp[NUM_A3], a2_var_tmp[NUM_A3];
	double z[NUM_A3];
	double z3_tmp[NUM_IN], a3_tmp[NUM_IN];
	double X[NUM_IN];
      ...
      //print message dependent on whether one or more buttons are pressed
      	if(button_data == 0b0000){} //do nothing
			...
			*/
			...
			Res = f_read(&fil,image_data_g, FileSize512, &NumBytesRead);
			if (Res) {
				xil_printf("ERROR: f_open\n");
				return XST_FAILURE;
			}
			...
			*/
			...
			block_cal(w2_mean, b2_mean, w2_var, b2_var, k, w3, b3, eps, psnr);
