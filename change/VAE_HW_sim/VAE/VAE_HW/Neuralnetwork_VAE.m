...
function [w12_mean,w12_var,w23,b2_mean,b2_var,b3,w12_mean_t,w12_var_t,w23_t,b2_mean_t,b2_var_t,b3_t,C] = Neuralnetwork_VAE(X,t,w12_mean,w12_var,w23,b2_mean,b2_var,b3,eta,epoch,L2func,L3func)
    ...
    for j=1:epoch
        ...
        a3 = 1./(1+exp(-z3));                   % 復元層（出力層）の出力a3（活性化関数：シグモイド関数）

        c = -t.*log(a3);                             % 交差（クロス）エントロピー誤差算出
        L = -1/2 *(1+log(a2_var) - a2_mean.^2 - a2_var);

        %c_z =

        c=sum(sum(c),2)+sum(sum(L),2);                        % 全データの誤差総和

        C(1,j) = c/size(X,2);                           % データ1個当たりの誤差を格納
    ...
    dCda2_var =  w23.' * (delta3) + a2_var-1./a2_var;

    dCdz2_mean = dCda2_mean;
    dCdz2_var = 1./(1+exp(-dCda2_var)); % ソフトプラスの微分
